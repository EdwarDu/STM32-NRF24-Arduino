 ...
 */
    ...
    #define MPU6050_I2C_PERIPH_RCC                          RCC_APB1Periph_I2C1
    #define MPU6050_I2C_GPIO                                RCC_APB2Periph_GPIOB

    #define i2c_write(a,b,c,d) I2C_Burst_Write(MPU6050_I2C_PORT, a, b, c, d)
    #define i2c_read(a,b,c,d) I2C_Burst_Read(MPU6050_I2C_PORT, a, b, c, d)
    ...
    // delay_ms get_ms functions are in delay.c(h)
    // #define HMC5883L_SECONDARY
    void MPU6050_I2C_Setup(void){
        RCC_APB1PeriphClockCmd(MPU6050_I2C_PERIPH_RCC, ENABLE);
        RCC_APB2PeriphClockCmd(MPU6050_I2C_GPIO, ENABLE);
        
        //RCC_APB1PeriphResetCmd(MPU6050_I2C_PERIPH_RCC, DISABLE);
        //RCC_APB1PeriphResetCmd(MPU6050_I2C_PERIPH_RCC, ENABLE);
        I2C_Config(MPU6050_I2C_PORT); // MPU6050 is connected to the I2C2
...
/*    #warning "[IN PROCESS] With HMC5883L_SECONDARY, HORRAY" */
#else
...
#define BIT_STBY_XYZG       (BIT_STBY_XG | BIT_STBY_YG | BIT_STBY_ZG)
...
#elif defined HMC5883L_SECONDARY
    // [TODO] [WANRING] Need to be checked again
    #define HMC5883L_FSR                (4095)
    #define HMC5883L_LOWBOUND           (-2048)
    #define HMC5883L_HIGHBOUND          (2047)
#endif
    ...
    #define HMC_AVG_SAMPLES_1           0x00    // 1 (Default)
    #define HMC_AVG_SAMPLES_2           0x20    // 2
    #define HMC_AVG_SAMPLES_4           0x40    // 4
    #define HMC_AVG_SAMPLES_8           0x60    // 8

    #define HMC_DO_RATE_0               0x00   // 0.75Hz
    #define HMC_DO_RATE_1               0x04   // 1.5Hz
    #define HMC_DO_RATE_2               0x08   // 3Hz
    #define HMC_DO_RATE_3               0x0C   // 7.5Hz
    #define HMC_DO_RATE_4               0x10   // 15Hz (Default)
    #define HMC_DO_RATE_5               0x14   // 30Hz
    #define HMC_DO_RATE_6               0x18   // 75Hz
    
    #define HMC_MM_0                    0x00    // Normal Measurement Mode (Default)
    #define HMC_MM_1                    0x01    // Positive bias configuration for XYZ axes
    #define HMC_MM_2                    0x02    // Negative bias configuration for XYZ axes

    #define HMC_REG_CFG_B               (0x01)
                                            // Recommended Sensor Field Range      Gain (LSb/Gauss)         Digital Resol(mG/LSb)       Output Range
    #define HMC_GAIN_0                  0x00    // +/- 0.88Ga                           1370                        0.73                    (-2048-2047) (0xF800 - 0x07FF)
    #define HMC_GAIN_1                  0x20    // +/- 1.3Ga                            1090(Default)               0.92                    (-2048-2047) (0xF800 - 0x07FF)
    #define HMC_GAIN_2                  0x40    // +/- 1.9Ga                            820                         1.22                    (-2048-2047) (0xF800 - 0x07FF)
    #define HMC_GAIN_3                  0x60    // +/- 2.5Ga                            660                         1.52                    (-2048-2047) (0xF800 - 0x07FF)
    #define HMC_GAIN_4                  0x80    // +/- 4.0Ga                            440                         2.27                    (-2048-2047) (0xF800 - 0x07FF)
    #define HMC_GAIN_5                  0xA0    // +/- 4.7Ga                            390                         2.56                    (-2048-2047) (0xF800 - 0x07FF)
    #define HMC_GAIN_6                  0xC0    // +/- 5.6Ga                            330                         3.03                    (-2048-2047) (0xF800 - 0x07FF)
    #define HMC_GAIN_7                  0xE0    // +/- 8.1Ga                            230                         4.35                    (-2048-2047) (0xF800 - 0x07FF)

    #define HMC_REG_MODE                (0x02)
    #define HMC_I2C_HS_ENABLE           0x80    // 3400kHz I2C
    #define HMC_OP_MODE_CONTINOUS       0x00    // Continous-Measuremnt Mode.
    #define HMC_OP_MODE_SINGLE          0x01    // Single-Measuremnt Mode.
    #define HMC_OP_MODE_IDLE            0x03    // Device is placed in idle mode

    #define HMC_REG_X_H                 (0x03)
    #define HMC_REG_X_L                 (0x04)
    #define HMC_REG_Z_H                 (0x05)
    #define HMC_REG_Z_L                 (0x06)
    #define HMC_REG_Y_H                 (0x07)
    #define HMC_REG_Y_L                 (0x08)

    #define HMC_REG_STATUS              (0x09)
    #define HMC_IS_LOCKED(x)            (x&0x02)
    #define HMC_IS_DRDY(x)              (x&0x01)

    #define HMC_REG_WHOAMI_1            (0x0A)
    #define HMC_REG_WHOAMI_2            (0x0B)
    #define HMC_REG_WHOAMI_3            (0x0C)

    #define HMC_IAM1                    0x48
    #define HMC_IAM2                    0x34
    #define HMC_IAM3                    0x33
#endif
...
const struct gyro_reg_s reg = {
    ...
    /* .gyro_cfg       = */ 0x1B,
    /* .accel_cfg      = */ 0x1C,
	/*  accel_cfg2     = */ 0xFF,   // not used by MPU6050
	/*  lp_accel_odr   = */ 0xFF,   // not used by MPU6050
    /* .motion_thr     = */ 0x1F,
    /* .motion_dur     = */ 0x20,
    /* .fifo_count_h   = */ 0x72,
    /* .fifo_r_w       = */ 0x74,
    ...
    /* .dmp_int_status = */ 0x39,
    /* .int_status     = */ 0x3A,
    /*  accel_intel    = */ 0xFF,  // not used by MPU6050
	/* .pwr_mgmt_1     = */ 0x6B,
    /* .pwr_mgmt_2     = */ 0x6C,
    /* .int_pin_cfg    = */ 0x37,
    /* .mem_r_w        = */ 0x6F,
...
#define MPU6500_MEM_REV_ADDR    (0x17)
    ...
    /* [TODO] May not be good to comment this
    if (int_param)
...
#if defined AK89xx_SECONDARY || defined HMC5883L_SECONDARY
#if defined AK89xx_BYPASS || defined HMC5883L_BYPASS
...
static int compass_self_test(void)
{
    ...
    int result = 0x07;
    short data[3];
    static short x_stp=-1, y_stp=-1, z_stp=-1;

    mpu_set_bypass(1);

    tmp[0] = HMC_OP_MODE_IDLE;
    if (i2c_write(st.chip_cfg.compass_addr, HMC_REG_MODE, 1, tmp))
        goto HMC_restore;
    delay_ms(10);
    
    tmp[0] = HMC_AVG_SAMPLES_8 | HMC_DO_RATE_4 | HMC_MM_1;
    if (i2c_write(st.chip_cfg.compass_addr, HMC_REG_CFG_A, 1, tmp))
       goto HMC_restore;
    tmp[0] = HMC_GAIN_5;
    if (i2c_write(st.chip_cfg.compass_addr, HMC_REG_CFG_B, 1, tmp))
        goto HMC_restore;
    tmp[0] = HMC_OP_MODE_CONTINOUS;
    if (i2c_write(st.chip_cfg.compass_addr, HMC_REG_MODE, 1, tmp))
        goto HMC_restore;

    do {
        delay_ms(6);
        if (i2c_read(st.chip_cfg.compass_addr, HMC_REG_STATUS, 1, tmp))
            goto HMC_restore;
        if (!HMC_IS_LOCKED(tmp[0]) && HMC_IS_DRDY(tmp[0]))
            break;
    } while (tries--);
    if (HMC_IS_LOCKED(tmp[0]) || !HMC_IS_DRDY(tmp[0]))
        goto HMC_restore;

    if (i2c_read(st.chip_cfg.compass_addr, HMC_REG_X_H, 6, tmp))
        goto HMC_restore;

    result = 0;
    data[0] = (short)(tmp[0] << 8) | tmp[1]; //X
    if ((data[0] > 575) || (data[0] < 243))
        result |= 0x01;
    data[2] = (short)(tmp[2] << 8) | tmp[3]; //Z
    if ((data[2] > 575) || (data[2] < 243))
        result |= 0x04;
    data[1] = (short)(tmp[4] << 8) | tmp[5]; //Y
    if ((data[1] > 575) || (data[1] < 243))
        result |= 0x02;
    
    if ((result == 0) && (x_stp != -1)){
        st.chip_cfg.mag_sens_adj[0] = ((long) data[0] << 8) / x_stp;
        st.chip_cfg.mag_sens_adj[1] = ((long) data[1] << 8) / y_stp;
        st.chip_cfg.mag_sens_adj[2] = ((long) data[2] << 8) / z_stp;
    } else {
        x_stp = data[0];
        y_stp = data[1];
        z_stp = data[2];
...
HMC_restore:
	tmp[0] = HMC_AVG_SAMPLES_8 | HMC_DO_RATE_6 |HMC_MM_0;
    i2c_write(st.chip_cfg.compass_addr, HMC_REG_CFG_A, 1, tmp);
	tmp[0] = HMC_GAIN_4;
    i2c_write(st.chip_cfg.compass_addr, HMC_REG_CFG_B, 1, tmp);
    mpu_set_bypass(0);
    return result;
}
#endif
...
static int setup_compass(void)
{
    ...
    return 0;
#elif defined HMC5883L_SECONDARY
    unsigned char data[4], hmc_addr;
    int result;
    
    mpu_set_bypass(1);

    result = i2c_read(HMC_I2C_ADDR, HMC_REG_WHOAMI_1, 3, data);
    if (result || data[0] != HMC_IAM1 || data[1] != HMC_IAM2 || data[2] != HMC_IAM3)
        return -1; // Can't read correct device IDs

    st.chip_cfg.compass_addr = HMC_I2C_ADDR;

    /* Setup of the HMC */
	data[0] = HMC_AVG_SAMPLES_8 | HMC_DO_RATE_6 |HMC_MM_0;
    if (i2c_write(st.chip_cfg.compass_addr, HMC_REG_CFG_A, 1, data))
        return -1;
    
	data[0] = HMC_GAIN_4;
    if (i2c_write(st.chip_cfg.compass_addr, HMC_REG_CFG_B, 1, data))
        return -1;
    
    // Put HMC to IDLE Mode
    data[0] = HMC_OP_MODE_IDLE;
    ...
    /* Set up master mode, master clock, and ES bit. */
    data[0] = 0x4D; // With I2C Fclk = 400kHz
    if (i2c_write(st.hw->addr, st.reg->i2c_mst, 1, data))
        return -1;
    ...
    /* Enable slave 0, 8-byte reads. 2*3Axes + 1Status*/
    data[0] = BIT_SLAVE_GROUP | BIT_SLAVE_BYTE_SW | BIT_SLAVE_EN | 6;
    if (i2c_write(st.hw->addr, st.reg->s0_ctrl, 1, data))
        return -1;
...
int mpu_get_compass_reg(short *data, unsigned long *timestamp)
{
...
#if defined HMC5883L_BYPASS
    tmp[8] = HMC_OP_MODE_SINGLE;
    if (i2c_write(st.chip_cfg.compass_addr, HMC_REG_MODE, 1, tmp+8))
        return -1;
    delay_ms(6);
    if (i2c_read(st.chip_cfg.compass_addr, HMC_REG_X_H, 6, tmp))
        return -1;
    //if (!HMC_IS_DRDY(tmp[6]) || HMC_IS_LOCKED(tmp[6]))
    //    return -2;
    data[0] = (tmp[0] << 8) | tmp[1];
    ...
    data[2] = (tmp[3] << 8) | tmp[2];
    data[1] = (tmp[5] << 8) | tmp[4];
#endif
    if (timestamp)
        get_ms(timestamp);
    return 0;
